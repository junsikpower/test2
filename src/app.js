...
(function (root, factory) {
  ...
  function createApp(opts) {
    ...
    function fireNotification(message) {
      // 재생 실패는 play() 의 Promise 거부 또는 동기 예외로도 감지된다 (FR-02).
      var soundPromise;
      try {
        soundPromise = beeper.play();
      } catch (e) {
        soundPromise = Promise.resolve(false);
      }
      ...
      Promise.resolve(soundPromise).then(
        function (ok) {
          // 소리 재생 실패가 감지되면 데스크톱 알림 호출 여부와 무관하게 탭 제목으로 보강한다.
          if (!ok) activateTitleFallback(message);
        },
        function () {
          // Promise 거부 = 재생 실패 → 탭 제목 변경으로 대체.
